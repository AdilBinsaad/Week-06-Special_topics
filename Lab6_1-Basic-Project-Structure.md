...
1. **Docker vs Native Setup**: อธิบายข้อดีของการใช้ Docker เปรียบเทียบกับการติดตั้ง ESP-IDF บน host system
   ...
   = Docker: สภาพแวดล้อมเหมือนกันทุกเครื่อง ติดตั้งง่าย ไม่กระทบระบบหลัก แยกโปรเจกต์ได้ดี

     Native: รันเร็วกว่า ยืดหยุ่นสูง ปรับแต่งระบบได้เต็มที่
   
2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary

   = ขั้นตอน build ESP-IDF ใน Docker คือ

      1. รัน container ที่มี ESP-IDF พร้อม toolchain

      2. แมปโฟลเดอร์โปรเจกต์เข้า container

      3. รันคำสั่ง build (idf.py build) ภายใน container

      4. คอมไพล์และลิงก์โค้ดเป็นไฟล์ binary

      5. ไฟล์ binary ถูกเก็บในโฟลเดอร์ที่แมปกลับมายัง host

      6. Docker ช่วยให้สภาพแวดล้อมการ build เหมือนกันทุกเครื่องและง่ายต่อการจัดการ dependencies
   
3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?

   = ไฟล์ CMakeLists.txt กำหนดขั้นตอนการคอมไพล์โปรเจกต์และรวมโค้ดย่อยต่างๆ ใน Docker ก็ทำงานเหมือน native เพราะ Docker มีสภาพแวดล้อมพร้อมใช้งานให้ CMake รันได้ปกติ

4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?

   = ไฟล์ .gitignore ช่วยกำหนดไฟล์หรือโฟลเดอร์ที่ไม่ต้องการให้ Git บันทึก เช่น ไฟล์บิลด์, ไฟล์ชั่วคราว หรือไฟล์คอนฟิกส่วนตัว ทำให้โปรเจกต์สะอาด และลดปัญหาการแชร์ไฟล์ที่ไม่จำเป็นหรือทำให้เกิดข้อผิดพลาดระหว่างทีมพัฒนา ESP32

5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?

   = ข้ออมูลที่ จะหายไปเมื่อ restart container คือ

ข้อมูลในไฟล์ระบบภายใน container ที่ไม่ได้เก็บใน volume หรือ mount

การตั้งค่าและไฟล์ที่สร้างขึ้นชั่วคราวภายใน container

ข้อมูลที่ จะอยู่ต่อหลัง restart container คือ

ไฟล์และข้อมูลที่เก็บใน Docker volumes หรือ bind mounts ที่แมปกับโฟลเดอร์บน host

ไฟล์ในระบบไฟล์ภายนอกที่เชื่อมต่อกับ container ผ่าน mount

สรุปคือ ถ้าจะเก็บข้อมูลถาวร ควรใช้ volume หรือ mount โฟลเดอร์จาก host ไปยัง container

6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system

   = Docker: สภาพแวดล้อมพร้อมใช้ ควบคุมง่าย เหมาะกับทีม ลดปัญหาเวอร์ชันต่างกัน

Native: ต้องติดตั้งเอง เหมาะกับผู้ใช้ที่ต้องการประสิทธิภาพสูงและปรับแต่งเต็มที่

Docker เน้นความสะดวกและสเถียร Native เน้นประสิทธิภาพและความยืดหยุ่น
